...
수정했습니다. 구 코드 12는 원본 스크립트에서 `none`을 의미하지만,
8월 이전에 존재할 수 없는 롱OFF로 표시되고 있어 사용자는 미갱신 기록을
흑동그라미로 표시하기로 결정했습니다. 아래 감사 도구는 운영 데이터를
변경하지 않고 코드 12의 상태와 원본 이력을 집계합니다.
...
`sql/06_apply_legacy_code12_black_circle.sql`은 감사 스테이징 뒤 같은 세션에서
수동으로만 실행하는 복구 SQL입니다. 전체 백업, 별도 DB 모의 실행,
`__EXPECTED_READY__` 건수 확인이 선행돼야 합니다. 대상의 원천 랭크 `none`
역시 현행 랭크 없음으로 고치고, 잠재 팝클과 클리어 레벨 캐시를 재계산합니다.

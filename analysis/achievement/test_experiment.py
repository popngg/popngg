...
class ExperimentTest(unittest.TestCase):
    def test_full_pipeline_exports_calibrated_intervals_and_holds_extrapolation(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(all(48 <= r['difficultyConstant'] <= 50 for r in visible))
            self.assertTrue(all(len(r['interval']) == 2 for r in visible))
            self.assertTrue(any('OUTSIDE_CALIBRATED_RANGE' in r['holdReasons'] for r in rows))
    ...
    def test_calibration_rejects_reversed_sparse_and_out_of_range_anchors(self):
        ...
        self.assertIsNone(convert(3, points))

...
class RatingImageControllerTest {
    @Test void returnsDownloadablePng()throws Exception{
        ...
        var playdata=mock(PlaydataQueryUseCase.class);var profiles=mock(UserProfileUseCase.class);var renderer=mock(TierListImageRenderer.class);
        var user=new PlaydataQueryResults.UserPlaydata("0000-0000-0001","User",0,0,0,List.of());
        var profile=mock(UserProfileResult.class);when(playdata.findUserPlaydata("0000-0000-0001")).thenReturn(user);when(profiles.get(new UserProfileQuery("0000-0000-0001"))).thenReturn(profile);when(renderer.render(any(),eq(user),eq(profile),eq(49),eq(RatingController.Metric.CPI))).thenReturn(new byte[]{1,2,3});
        var response=new RatingImageController(ratings,playdata,profiles,renderer).image(49,RatingController.Metric.CPI,"0000-0000-0001");
    ...
    @Test void rejectsBotAndMalformedIds(){
        var controller=new RatingImageController(mock(RatingQuery.class),mock(PlaydataQueryUseCase.class),mock(UserProfileUseCase.class),mock(TierListImageRenderer.class));

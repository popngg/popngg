...
class AuthControllerTest {
    ...
    private final AuthController controller = new AuthController(authenticate, passwordReset, register);

    @Test
    void loginReturnsBodyAndSecureCookie() {
        ...
        assertThat(response.getData().accessToken()).isEqualTo("token");
    ...
    @Test
    void delegatesPasswordResetAndReturnsAuthCheck() {
        ...
        var check = controller.check(principal).getData();
        assertThat(check.poptomoId()).isEqualTo(ID);
        assertThat(check.role()).isEqualTo("USER");
    }
